#!/usr/bin/env python3
"""Offline Unigram adapter for an operator-sealed local tokenizer and private key.
...
def _validate_configuration(value: Any) -> dict[str, Any]:
    ...
    if any(
        ...
        raise ValueError("configuration_invalid")
    numeric = ("alpha", "fraction", "strength", "threshold")
    if any(
    ...
    if (
        ...
        or not math.isclose(
            ...
            NormalDist().inv_cdf(1.0 - float(value["alpha"])),
            rel_tol=0.0,
            abs_tol=1e-12,
        )
        or not 0.0 < float(value["alpha"]) < 0.5
        or not 0.0 < float(value["fraction"]) < 1.0
        or not 1 <= int(float(value["fraction"]) * vocab_size) < vocab_size
        or float(value["strength"]) < 0.0
        or value.get("tokenizer_type") != "transformers_local_files_v1"
        or not _valid_public_identifier(value.get("tokenizer_revision"))
        or minimum > vocab_size
...
def _load_key(path: Path, key_id: str) -> int:
...
def _load_runtime(
    ...
    source = upstream_dir.absolute() / "gptwm.py"
    if (
    ...
    if str(numpy.__version__) != configuration["numpy_version"]:
    ...
    if len(tokenizer) != configuration["vocab_size"]:
        raise ValueError("tokenizer_mismatch")
    # The pinned source imports this historical base class even though its
    # detector does not use it. A compatible Transformers runtime must expose
    # it; no remote custom code is accepted as a substitute.
    if not hasattr(transformers, "LogitsWarper"):
        raise ValueError("runtime_mismatch")
    spec = importlib.util.spec_from_file_location("_dewatermark_operator_unigram", source)
...
def handle(
    ...
    *,
    ...
    try:
        key = _load_key(key_file, str(configuration["key_id"]))
        ...
        raw_z = float(detector.unidetect(list(unique_ids)))
        finite_population = math.sqrt(
            1.0 - (effective_tokens - 1.0) / (int(configuration["vocab_size"]) - 1.0)
        )
        if finite_population <= 0.0:
            z_score = 0.0
            p_value = 1.0
        else:
            z_score = raw_z / finite_population
